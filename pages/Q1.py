...
if "base_info" not in st.session_state:
    st.session_state.base_info = func.load_base_info("q1")
...
with st.expander("👤 Information", expanded=True):
    ...
    with col1:
        age = st.number_input("Mon Age", min_value=18, max_value=100, value=datetime.today().year - int(st.session_state.base_info["Client"]["PatClientDetail"][0]["dateNaissance"][:4]))
        situation = st.selectbox("Ma Situation Personnelle", ["Célibataire", "Union Libre", "Pacsé(e)", "Marié(e)"], index=["Célibataire", "Union Libre", "Pacsé(e)", "Marié(e)"].index(st.session_state.base_info["Client"]["PatClientDetail"][0]["typeUnion"]))
        charge = st.number_input("Mes dépenses courantes mensuelles (€)", min_value=0, step=100, value=int(st.session_state.base_info["Cashflow"]["PatCashflowDetail"][0]["depensesCourantes"]))

    with col2:
        loyer = st.number_input("Mon loyer mensuel (€)", min_value=0, step=100, value=int(st.session_state.base_info["Cashflow"]["PatCashflowDetail"][0]["loyerHabitationPrincipale"]))
        emprunt = st.number_input("Mon emprunt étudiant (€)", min_value=0, step=1000, value=int(st.session_state.base_info["Emprunt"]["PatEmpruntDetail"][0]["montantRestantDu"]))

    with col3:
        fin = st.number_input("Mon épargne Livret A (€)", min_value=0, step=1000, value=int(st.session_state.base_info["Fin"]["PatFinDetail"][0]["value"]))
        salaire = st.number_input("Mon salaire brut annuel (€)", min_value=0, step=1000, value=int(st.session_state.base_info["Cashflow"]["PatCashflowDetail"][0]["revenusActivite"])) 
    
    # Save modifications
    if st.button("💾 Enregistrer"):
        st.session_state.base_info["Client"]["PatClientDetail"][0]["dateNaissance"] = f"{datetime.today().year - age}-01-01"
        st.session_state.base_info["Client"]["PatClientDetail"][0]["typeUnion"] = situation
        st.session_state.base_info["Emprunt"]["PatEmpruntDetail"][0]["montantRestantDu"] = emprunt
        st.session_state.base_info["Emprunt"]["PatEmpruntDetail"][0]["quotePart"] = emprunt
        st.session_state.base_info["Fin"]["PatFinDetail"][0]["value"] = fin
        st.session_state.base_info["Fin"]["PatFinDetail"][0]["quotePart"] = fin
        st.session_state.base_info["Cashflow"]["PatCashflowDetail"][0]["loyerHabitationPrincipale"] = loyer
        st.session_state.base_info["Cashflow"]["PatCashflowDetail"][0]["depensesCourantes"] = charge
        st.session_state.base_info["Cashflow"]["PatCashflowDetail"][0]["revenusActivite"] = salaire
        
        with open("json/q1.json", "w", encoding="utf-8") as f:
            json.dump(st.session_state.base_info, f, ensure_ascii=False, indent=4)
...
with st.expander("🕒 BILAN KLEMO", expanded=True):
    ...
    if st.button("LANCER LA SIMULATION KLEMO"):
        ...
        json_proj, time_elapsed = func.call_api(st.session_state.base_info,func.FILL_SCORE_URL)
...
with st.expander("💡 RECOS KLEMO", expanded=True):
    ...
    if st.session_state.json_synth_id: 
        ...
        paramObj = st.text_area(":black_nib: entrer des paramètres d'objectif (optionnel):",value='{"debut":"2029-09-30", "montantRegulier":400,"horizon":24}', height=200)
        ...
        # if st.button("Send Request to our API StratPat"):
            if st.session_state.json_synth_id: 
                ...
                if init_strat:
                    ...
                    func.display_strat_output(st.session_state.payload_strat, strat_output,5)
